...
export default function CreatorDashboard() {
  ...
  const { data: tipEvents, isLoading: isLoadingEvents } = useScaffoldEventHistory({
    ...
    fromBlock: 0n, // Scan from beginning or a specific block
  ...
  const handleWithdraw = async () => {
    try {
      ...
      // notification handles success/error automatically via scaffold hook usually, or we can add manual if needed
    } catch (e) {
      console.error(e);
    }
  };
    ...
    <div className="flex flex-col gap-8 py-8 px-4 sm:px-12 max-w-7xl mx-auto">
      ...
      <div className="card bg-base-100 shadow-xl border border-secondary/20">
        <div className="card-body flex-row items-center justify-between">
          <div>
            <h2 className="card-title text-base-content/60 text-sm">Available to Withdraw</h2>
            <p className="text-4xl font-bold text-secondary">
              ${availableBalance ? formatUnits(availableBalance, 6) : "0.00"}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="card bg-base-100 shadow-xl border border-base-300">
          <div className="card-body">
            <h2 className="card-title text-base-content/60 text-sm">Total Tipped (Lifetime)</h2>
        ...
        <div className="card bg-base-100 shadow-xl border border-base-300">
          <div className="card-body">
            ...
            {isLoadingEvents ? (
                  ...
                  <tbody>
                    {tipEvents?.slice(0, 10).map((event: any, i: number) => (
                      <tr key={i} className="hover">
                        ...
                        <td className="font-mono text-success">+${formatUnits(event.args.amount || 0n, 6)}</td>
                        <td>
                          <a
              ...
              <div className="text-center py-8 text-base-content/50">No tips received yet.</div>
            )}
          </div>
        </div>
        ...
        <div className="card bg-base-100 shadow-xl border border-base-300">
          <div className="card-body">
            ...
            {(stats.leaderboard.length || 0) > 0 ? (
                  ...
                  <tbody>
                    {stats.leaderboard.slice(0, 10).map((entry, i) => (
                      <tr key={i} className={i < 3 ? "bg-base-200/50" : ""}>
                        ...
                        <td className="font-bold">${formatUnits(entry.amount, 6)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="text-center py-8 text-base-content/50">No data yet.</div>
            )}
          </div>
        </div>
      </div>
