...
// Create a static client for Sepolia ENS resolution
...
export const CreatorList = ({ onSelectCreator }: CreatorListProps) => {
  ...
  // Effect to resolve ENS on Sepolia when the custom address changes
  useEffect(() => {
    let active = true;
  ...
  }, [searchTerm, isCustomAddress]);

  const showCustomOption =
    isCustomAddress && !filteredCreators.find(c => c.wallet?.toLowerCase() === searchTerm.toLowerCase());
        ...
        {showCustomOption && (
          <div
            onClick={() =>
              onSelectCreator({
                id: "custom",
                name: ensName || "Custom Address", // Use ENS name if available
                wallet: searchTerm,
                description: "Tip this specific address",
                tags: ["Custom"],
                avatar: ensAvatar || undefined, // Pass resolved avatar
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-sm text-base-content">{ensName || "Tip Address"}</h3>
